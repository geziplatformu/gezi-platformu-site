(()=>{
  ...
  style.textContent=`
    ...
    .gp-autumn-leaf{position:absolute;top:-12vh;left:var(--x);width:var(--size);height:var(--size);opacity:0;will-change:transform,opacity;filter:drop-shadow(0 2px 3px rgba(59,35,16,.22)) saturate(1.18);animation:gpLeafFall var(--duration) linear var(--delay) forwards}
    ...
    @keyframes gpLeafFall{
      ...
      76%{transform:translate3d(var(--drift2),78vh,0) rotate(245deg);opacity:calc(var(--opacity) * .96)}
    ...
    @media(max-width:700px){.gp-autumn-warmth{background:linear-gradient(180deg,rgba(173,94,31,.022),rgba(255,255,255,0) 48%)}.gp-autumn-leaf{filter:drop-shadow(0 1px 2px rgba(59,35,16,.18)) saturate(1.16)}}
  `;
  ...
  const colors=['#a94d18','#c9651d','#d9851f','#9d5622','#e0a12a','#bb571d','#cf7320'];
  const paths=[
  ...
  function makeLeaf(initial=false){
    ...
    leaf.innerHTML=`<svg viewBox="0 0 100 100" focusable="false" aria-hidden="true"><path d="${path}" fill="${color}"/><path d="M50 18C49 39 50 66 48 94" fill="none" stroke="rgba(92,42,17,.55)" stroke-width="3" stroke-linecap="round"/></svg>`;
